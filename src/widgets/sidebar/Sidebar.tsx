...
import { BrandLogo, HamburgerButton } from '@/shared/ui'
...
export const Sidebar = ({ open, onToggle, className }: SidebarProps) => {
  ...
  return (
    <SidebarContainer className={className} open={open}>
      <div className={clsx('flex mb-4 flex-wrap justify-end')}>
        <div className='basis-1/2 flex justify-end'>
          <HamburgerButton open={open} onClick={onToggle} />
        </div>
      </div>

      <div className='flex items-center mb-2 ml-[3px] overflow-hidden'>
        <div className='mr-2'>
          <Avatar />
        </div>
        <span className={clsx(!open && 'hidden', 'whitespace-nowrap')}>
          SomeName Some
        </span>
      </div>

      <SidebarSection>
        <Navigation open={open} items={navigationItems} />
      </SidebarSection>
